/*
 * Question 8 (Java) : Versions recursives de plus et moins.
 *
 * Reprend la Q7 et y AJOUTE deux methodes :
 *   static Maillon plusRec (Maillon a, Maillon b);
 *   static Maillon moinsRec(Maillon a, Maillon b);
 * qui realisent l'addition et la soustraction sous une forme
 * purement recursive (sans boucle while).
 *
 * Les versions iteratives plus() et moins() de la Q6 sont conservees
 * intactes : elles coexistent avec les versions recursives. Le main
 * verifie que les deux versions produisent exactement le meme
 * resultat sur les exemples donnes.
 *
 * Idee de la version recursive : a chaque appel, on regarde les
 * deux tetes a et b, on decide ce que produit la tete du resultat,
 * et on delegue le calcul du reste a un appel recursif sur les
 * queues. Cas de base : l'une des deux listes est vide.
 *
 *   plusRec(null, b) = copie(b)
 *   plusRec(a, null) = copie(a)
 *   plusRec(a, b) avec a.expo > b.expo
 *     = (a.coef, a.expo) :: plusRec(a.suivant, b)
 *   plusRec(a, b) avec a.expo < b.expo
 *     = (b.coef, b.expo) :: plusRec(a, b.suivant)
 *   plusRec(a, b) avec exposants egaux
 *     soit s = a.coef + b.coef :
 *       si s == 0 : plusRec(a.suivant, b.suivant)
 *       sinon     : (s, a.expo) :: plusRec(a.suivant, b.suivant)
 *
 * Le code resultant est plus court et plus mathematique.
 *
 * --- Q7 ---
 * Recyclage de la memoire (garbage collector)
...
public class Analyseur {
    ...
    /* ==================================================================
       Versions recursives de plus et moins (Q8)
       ==================================================================
       Memes operations que plus()/moins() de la Q6, mais sans boucle.
       Chaque appel decide d'un seul maillon du resultat (la tete) et
       delegue le reste a un appel recursif.
       ================================================================== */

    /* Addition recursive de deux polynomes. */
    private static Maillon plusRec(Maillon a, Maillon b) {
        /* Cas de base : si l'une des listes est vide, le resultat est
           une COPIE de l'autre. On utilise copie() pour respecter la
           regle "ne pas reutiliser les maillons d'entree". */
        if (a == null)
            return copie(b);
        if (b == null)
            return copie(a);

        /* a.expo > b.expo : la tete du resultat vient de a, et le
           reste se calcule recursivement entre la queue de a et b. */
        if (a.expo > b.expo)
            return new Maillon(a.coef, a.expo,
                               plusRec(a.suivant, b));

        /* a.expo < b.expo : symetrique. */
        if (a.expo < b.expo)
            return new Maillon(b.coef, b.expo,
                               plusRec(a, b.suivant));

        /* Exposants egaux : on additionne les coefficients. Si la
           somme est nulle, ce monome disparait du resultat ; sinon
           il devient la tete. Dans les deux cas, on avance des deux
           cotes. */
        double s = a.coef + b.coef;
        Maillon queue = plusRec(a.suivant, b.suivant);
        if (s == 0)
            return queue;
        return new Maillon(s, a.expo, queue);
    }

    /* Soustraction recursive : meme structure que plusRec, mais on
       nie systematiquement les coefficients qui viennent de b. */
    private static Maillon moinsRec(Maillon a, Maillon b) {
        if (a == null)
            return neg(b);          /* 0 - b = -b */
        if (b == null)
            return copie(a);        /* a - 0 =  a */

        if (a.expo > b.expo)
            return new Maillon(a.coef, a.expo,
                               moinsRec(a.suivant, b));

        if (a.expo < b.expo)
            /* La tete du resultat vient de b mais avec coef negre. */
            return new Maillon(-b.coef, b.expo,
                               moinsRec(a, b.suivant));

        /* Exposants egaux : on soustrait. */
        double s = a.coef - b.coef;
        Maillon queue = moinsRec(a.suivant, b.suivant);
        if (s == 0)
            return queue;
        return new Maillon(s, a.expo, queue);
    }

    /* Test : compare deux polynomes maillon par maillon. Renvoie true
       s'ils sont identiques, false sinon. Utilise pour verifier que
       les versions iteratives et recursives produisent des resultats
       egaux. */
    private static boolean egaux(Maillon p, Maillon q) {
        while (p != null && q != null) {
            if (p.coef != q.coef || p.expo != q.expo)
                return false;
            p = p.suivant;
            q = q.suivant;
        }
    ...
    */
    ...
    public static void main(String[] args) throws IOException {
        ...
        /* Versions iteratives (Q6). */
        Maillon sommeIter = plus(a, b);
        Maillon diffIter  = moins(a, b);
        /* Versions recursives (Q8). */
        Maillon sommeRec  = plusRec(a, b);
        Maillon diffRec   = moinsRec(a, b);

        System.out.print("A + B (iteratif) = "); affiche(sommeIter); System.out.println();
        System.out.print("A + B (recursif) = "); affiche(sommeRec);  System.out.println();
        System.out.println("  -> resultats "
            + (egaux(sommeIter, sommeRec) ? "identiques" : "DIFFERENTS"));

        System.out.print("A - B (iteratif) = "); affiche(diffIter); System.out.println();
        System.out.print("A - B (recursif) = "); affiche(diffRec);  System.out.println();
        System.out.println("  -> resultats "
            + (egaux(diffIter, diffRec) ? "identiques" : "DIFFERENTS"));
