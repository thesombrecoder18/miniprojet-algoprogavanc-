/*
 * Question 8 : Versions recursives de plus et moins.
 *
 * Ce fichier reprend la Q7 et y AJOUTE deux fonctions :
 *   POINTEUR plusRec (POINTEUR a, POINTEUR b);
 *   POINTEUR moinsRec(POINTEUR a, POINTEUR b);
 * qui realisent l'addition et la soustraction de deux polynomes
 * sous une forme purement recursive (sans boucle while).
 *
 ...
 *
 * Le code resultant est plus court et plus mathematique : chaque
 * branche du if correspond a un cas de la definition.
 *
 * (Suit la documentation Q7 ci-dessous, conservee intacte.)
 *
 * --- Q7 ---
 * Recyclage de la memoire selon l'algorithme
...
/* ==================================================================
   ...
   ==================================================================
   Ces fonctions realisent les memes operations que plus() et moins()
   de la Q6, mais sans boucle. Chaque appel decide d'un seul maillon
   du resultat (la tete) et delegue le reste a un appel recursif.
   ================================================================== */

/* Addition recursive de deux polynomes. */
static POINTEUR plusRec(POINTEUR a, POINTEUR b) {
    ...
    if (a->expo > b->expo)
        return nouveauMaillon(a->coef, a->expo,
                              plusRec(a->suivant, b));

    /* a->expo < b->expo : symetrique. */
    if (a->expo < b->expo)
        return nouveauMaillon(b->coef, b->expo,
                              plusRec(a, b->suivant));

    /* Exposants egaux : on additionne les coefficients. Si la somme
       est nulle, ce monome disparait du resultat ; sinon il devient
       la tete. Dans les deux cas, on avance des deux cotes. */
    double s = a->coef + b->coef;
    POINTEUR queue = plusRec(a->suivant, b->suivant);
    if (s == 0)
        return queue;
...
static POINTEUR moinsRec(POINTEUR a, POINTEUR b) {
    /* Cas de base. */
    if (a == NULL)
        return neg(b);          /* 0 - b = -b */
    if (b == NULL)
    ...
    double s = a->coef - b->coef;
    POINTEUR queue = moinsRec(a->suivant, b->suivant);
    if (s == 0)
        return queue;
    return nouveauMaillon(s, a->expo, queue);
}

/* Test : compare deux polynomes maillon par maillon. Renvoie 1 si
   identiques, 0 sinon. Utilise pour verifier que les versions
   iteratives et recursives produisent des resultats egaux. */
static int egaux(POINTEUR p, POINTEUR q) {
    while (p != NULL && q != NULL) {
...
int main(void) {
    ...
    POINTEUR sommeIter   = plus(a, b);
    POINTEUR diffIter    = moins(a, b);
    /* Versions recursives (Q8). */
    POINTEUR sommeRec    = plusRec(a, b);
    POINTEUR diffRec     = moinsRec(a, b);

    printf("A + B (iteratif) = "); affiche(sommeIter); printf("\n");
    printf("A + B (recursif) = "); affiche(sommeRec);  printf("\n");
    printf("  -> resultats %s\n",
           egaux(sommeIter, sommeRec) ? "identiques" : "DIFFERENTS");

    printf("A - B (iteratif) = "); affiche(diffIter); printf("\n");
    printf("A - B (recursif) = "); affiche(diffRec);  printf("\n");
    printf("  -> resultats %s\n",
